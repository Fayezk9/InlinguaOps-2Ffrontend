...
function findMonthlySheetTitle(
...
export default function AddOrdersToListDialog({
  open,
  onOpenChange,
  apiBase,
  sheetId,
  ...
  const loadNew = async () => {
    ...
    try {
      ...
      const list: OrdersListRow[] = Array.isArray(data.results)
        ? data.results
        : [];
      setRows(list);
      setPage(1);
  ...
  const toggleAllOnPage = () => {
    if (allCheckedOnPage) {
      ...
      setSelectedMap((prev) => ({
        ...prev,
        ...Object.fromEntries(paged.map((r) => [r.id, r])),
      }));
    }
  };
  ...
  const onAddToList = async () => {
    if (selectedRows.length === 0) {
      toast({
        ...
        description: "Select one or more orders",
        variant: "destructive",
      });
      return;
    }
    try {
      ...
      for (const r of selectedRows) {
        const res = await fetch(`${apiBase}/orders/search`, {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            searchCriteria: { orderNumber: String(r.id) },
          }),
        });
        ...
        const match =
          arr.find((it) => Number(it?.wooOrder?.id) === Number(r.id)) || arr[0];
      ...
      try {
        ...
        if (typeof window !== "undefined") {
          window.dispatchEvent(
            new CustomEvent("orders-added-to-sheet", {
              detail: { ids: newIds },
            }),
          );
        }
      ...
      <DialogContent className="max-w-5xl">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <Button
              variant={tab === "old" ? "default" : "outline"}
              size="sm"
              onClick={() => setTab("old")}
            >
              Old
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={9} className="text-red-500 text-sm">
                    {error}
                  </TableCell>
                </TableRow>
              ) : (
                  ...
                  <TableRow key={r.id}>
                    <TableCell className="w-10">
                      <Checkbox
                        ...
                        onCheckedChange={() => toggleId(r.id, r)}
                      />
                    </TableCell>
        ...
        <DialogFooter className="gap-2">
          ...
          <Button onClick={onAddToList} disabled={selectedRows.length === 0}>
