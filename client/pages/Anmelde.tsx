...
export default function Anmelde() {
  ...
  const generateRegistrationPDFs = async () => {
    ...
    try {
      ...
      const response = await fetch(JAVA_ACTION_ENDPOINTS.REGISTRATION_PDF, {
        method: 'POST',
        headers: {
      ...
      <Card className="border border-border bg-card text-card-foreground">
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Generate registration confirmation PDFs for orders
          </p>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <p className="text-xs text-muted-foreground">
              Enter order numbers separated by commas, spaces, or new lines. Only numeric values will be processed.
            </p>
          </div>
          ...
          <div>
            <Button
              onClick={generateRegistrationPDFs}
            ...
            >
              {isGenerating ? (
                <>
                  ...
                  Generating Registration PDFs...
                </>
              ) : (
                <>
                  <FileText className="h-4 w-4 mr-2" />
                  Generate {validOrderCount} Registration PDF{validOrderCount !== 1 ? 's' : ''}
                </>
              )}
            </Button>
          </div>
                  ...
                  <AlertDescription className="mt-2">
                    <div className="flex gap-4 text-sm">
                      {lastResult.processedCount !== undefined && (
                        <span>Processed: {lastResult.processedCount}</span>
                      )}
          ...
          <Alert>
            ...
            <AlertDescription>
              ...
              <ul className="list-disc list-inside mt-2 space-y-1 text-sm">
                <li>Enter the order numbers you want to generate PDFs for</li>
                <li>The system will fetch order details from WooCommerce</li>
                <li>Registration confirmation PDFs will be generated automatically</li>
                <li>Files will be saved to your configured output directory</li>
              </ul>
            </AlertDescription>
          </Alert>
