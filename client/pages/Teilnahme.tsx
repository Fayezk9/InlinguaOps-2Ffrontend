...
import { Loader2, Award, CheckCircle, AlertCircle, Upload } from "lucide-react";
...
export default function Teilnahme() {
  ...
  const generateParticipationPDFs = async () => {
    ...
    try {
      ...
      const response = await fetch(JAVA_ACTION_ENDPOINTS.PARTICIPATION_PDF, {
        method: 'POST',
        headers: {
      ...
      <Card className="border border-border bg-card text-card-foreground">
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Generate participation certificates for completed exams
          </p>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <p className="text-xs text-muted-foreground">
              Enter order numbers for participants who have completed their exams.
            </p>
          </div>
          ...
          <div>
            <Button
              onClick={generateParticipationPDFs}
              disabled={isGenerating || validOrderCount === 0 || !javaStatus?.javaAvailable}
              className="w-full"
              size="lg"
            >
              {isGenerating ? (
                ...
                <>
                  ...
                  Generate {validOrderCount} Participation Certificate{validOrderCount !== 1 ? 's' : ''}
                </>
              )}
            </Button>
          </div>
          ...
          {lastResult && (
            <div>
              <Label className="text-sm font-medium">Last Generation Result</Label>
                  ...
                  <AlertDescription className="mt-2">
                    <div className="flex gap-4 text-sm">
                      {lastResult.processedCount !== undefined && (
                        <span>Certificates generated: {lastResult.processedCount}</span>
                      )}
                ...
                {lastResult.outputPath && (
                  <AlertDescription className="mt-2">
                    <div className="text-sm">
                      <strong>Output:</strong> {lastResult.outputPath}
                    </div>
                  </AlertDescription>
                )}
              </Alert>
          ...
          <Alert>
            ...
            <AlertDescription>
              ...
              <ul className="list-disc list-inside mt-2 space-y-1 text-sm">
                <li>Enter order numbers for participants who completed their exams</li>
                <li>The system will fetch participant details from WooCommerce</li>
                <li>Professional participation certificates will be generated</li>
                <li>Certificates are saved in the "Teilnahme" subfolder by exam date</li>
                <li>Each certificate includes participant info and exam details</li>
              </ul>
            </AlertDescription>
          </Alert>
