...
export const uploadRegistrationPdfTemplate: RequestHandler = async (
  req,
  res,
) => {
  try {
    ...
    return res.json({
      message: "PDF template uploaded",
      path: TEMPLATE_PDF_PATH,
      stored: "db",
    });
  } catch (e: any) {
    return res
      .status(500)
      .json({ message: e?.message || "Failed to upload PDF template" });
  }
};

export const getRegistrationPdfTemplateStatus: RequestHandler = async (
  _req,
  res,
) => {
  try {
    ...
    if (dbB64) {
      return res.json({
        exists: true,
        size: Math.floor((dbB64.length * 3) / 4),
        source: "db",
      });
    }
    ...
    return res
      .status(500)
      .json({ message: e?.message || "Failed to get PDF template status" });
  }
};

export const validateRegistrationPdfTemplate: RequestHandler = async (
  _req,
  res,
) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: e?.message || "Failed to validate PDF template" });
  }
};
...
const generateRequest = z.object({
  ...
  templateType: z.enum(["registration", "participation"]).optional(),
});

export const uploadPdfTemplateToDb: RequestHandler = async (req, res) => {
  try {
    const schema = z.object({
      type: z.enum(["registration", "participation"]),
      contentBase64: z.string().min(1),
    });
    ...
    return res
      .status(500)
      .json({ message: e?.message || "Failed to upload template" });
  }
};
...
export const getPdfTemplateStatus: RequestHandler = async (req, res) => {
  try {
    ...
    if (type !== "registration" && type !== "participation")
    ...
    return res
      .status(500)
      .json({ message: e?.message || "Failed to get status" });
  }
};
...
export const validatePdfTemplateFromDb: RequestHandler = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: e?.message || "Failed to validate template" });
  }
};
...
export const generateRegistrationPdf: RequestHandler = async (req, res) => {
  try {
    ...
    if (dbB64) {
      try {
        templateBuf = Buffer.from(dbB64, "base64");
      } catch {}
    }
    if (!templateBuf) {
    ...
    const examKind =
      deriveExamKind() || extractFromMeta(meta, META_KEYS_EXAM_KIND) || "";
    ...
    const examTime =
      examPartLc.includes("mündlich") || examPartLc.includes("muendlich")
        ? "14:30 Uhr"
        : "09:00 Uhr";
    ...
    const birthCountryRaw =
      extractFromMeta(meta, META_KEYS_BIRTH_COUNTRY) || "";
    const toAlpha3 = (s: string): string => {
      const v = (s || "").trim();
      if (!v) return "";
      const up = v.toUpperCase();
      try {
        if (up.length === 3 && countries.alpha3ToAlpha2(up)) return up;
      } catch {}
      try {
        if (up.length === 2) {
          const a3 = countries.alpha2ToAlpha3(up as any);
          if (a3) return a3;
        }
      } catch {}
      try {
        const de3 = countries.getAlpha3Code(v, "de");
        if (de3) return de3;
      } catch {}
      try {
        const en3 = countries.getAlpha3Code(v, "en");
        if (en3) return en3;
      } catch {}
      return "";
    };
    ...
    const toDisplayNat = (code3: string, fallbackRaw: string): string => {
      ...
      try {
        if (code3) {
          ...
          if (a2) {
            try {
              const dn = new (Intl as any).DisplayNames(["de"], {
                type: "region",
              });
              const name = dn?.of?.(a2);
              if (name) return name;
      ...
      if (/^[A-Z]{2,3}$/.test(maybe)) {
        try {
          const a2 =
            maybe.length === 2 ? maybe : countries.alpha3ToAlpha2(maybe);
          if (a2) {
            try {
              const dn = new (Intl as any).DisplayNames(["de"], {
                type: "region",
    ...
    const fullAddressCombined = [
      ...
      .filter(Boolean)
      .join("\n");
    const fullCity = [billing?.postcode || "", billing?.city || ""]
      .filter(Boolean)
      .join(" ")
      .trim();
    const data: Record<string, any> = {
      ...
      fullCity,
      streetHouse: (() => {
        const a = [billing?.address_1 || "", billing?.address_2 || ""]
          .filter(Boolean)
          .join(" ")
          .trim();
        if (a) return a;
        const lines = String(fullAddressCombined || "")
          .split("\n")
          .map((s) => s.trim())
          .filter(Boolean);
    ...
    data.exam =
      `${data.examKind || ""}${data.examPart ? ` (${data.examPart})` : ""}`.trim();
    ...
    data.streetHouse =
      String(data.streetHouse || "").trim() ||
      [data.address1, data.address2].filter(Boolean).join(" ").trim() ||
      deriveStreet(data.fullAddress, data.fullCity);
    ...
    const norm = (s: string) =>
      s
        ...
        .toUpperCase()
        .replace(/[^A-Z0-9]+/g, "");
    const aliasNorm: Record<string, string> = Object.fromEntries(
      Object.entries(aliasMap).map(([k, v]) => [norm(k), v]),
    );
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="registration-${data.orderNumber}.pdf"`,
    );
    return res.send(Buffer.from(out));
  } catch (e: any) {
    return res
      .status(500)
      .json({ message: e?.message || "Failed to generate PDF" });
  }
};
